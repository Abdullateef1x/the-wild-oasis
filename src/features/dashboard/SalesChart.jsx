...
function SalesChart({bookings, numDays}) {
  ...
  return (
    <StyledSalesChart >
      <Heading as="h2">Sales from {format(allDates.at(0), "dd MMM yyyy")}&mdash;{format(allDates.at(-1), "dd MMM yyyy")}</Heading>
      ...
      <YAxis unit='$'tick={{fill: colors.text}} tickLine={{stroke: colors.text}}/>
        ...
        <Area dataKey="totalSales" type="monotone" stroke={colors.totalSales.stroke} fill={colors.totalSales.fill} strokeWidth={2} 
        name="Total sales" unit='5'/>
         <Area dataKey="extrasSales" type="monotone" stroke={colors.extrasSales.stroke} fill={colors.extrasSales.fill} strokeWidth={2} 
        name="Extras sales" unit='5'/>
      </AreaChart>
      </ResponsiveContainer>
    </StyledSalesChart>
  );
